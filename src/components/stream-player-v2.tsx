...
export function StreamPlayer({isHost = false}) {
    ...
    const [background, setBackground] = useState<string>("not"); // 创建状态
    ...
    const remoteVideoTracks = videoTracks.filter(
        (t) => {
            return t.publication.trackName === t.participant.identity + 'canvas';
        }
    );
        ...
        <div className="relative h-full max-h-screen w-full bg-black flex flex-col overflow-y-auto">
            ...
            <div className="w-full overflow-x-auto flex-shrink-0" style={{height: "30%"}}>
                <div className="flex h-full space-x-2 p-2">
                    <video ref={localVideoEl} playsInline
                           style={{
                               display: "none",
                           }}/>
                    <canvas
                            ...
                            <div className="absolute bottom-2 right-2 z-20">
                                ...
                                >
                                    {roomMetadata?.creator_identity === t.participant.identity ? (
                                        <span
                                            className="inline-block px-1.5 py-0.5 text-xs font-semibold text-white bg-red-9 rounded mr-1">主播</span>
                                    ) : (
                                        <span
                                            className="inline-block px-1.5 py-0.5 text-xs font-semibold text-gray-700 bg-gray-300 rounded mr-1">观众</span>
                                    )}
            ...
            {/* 修改后的白板区域 */}
            <div className="bg-gray-900 border-t border-gray-700 relative flex justify-center items-center"
                        ...
                        {roomName && canHost && (
                            <Flex gap="2">
                                ...
                                <BackgroundContext.Provider value={{background, setBackground}}>
                                    <VideoSegmentation videoRef={localVideoEl} canvasRef={canvasRef} isHost={isHost}/>
                            ...
                            <div className="relative">
                                {showNotification && (
                                    <div className="absolute flex h-3 w-3 -top-1 -right-1">
                                        <span
                                            className="animate-ping absolute inline-flex h-full w-full rounded-6 bg-accent-11 opacity-75"></span>
